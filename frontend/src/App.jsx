      ...
      {result && (
        <div className="search-result-groq">
          ...
          <div className="search-result-groq-header">
            ...
            <span className="search-result-groq-badge">Powered by Codevidhya</span>
...
function FloatingChat() {
  ...
  const [messages, setMessages] = useState([
    { role: "assistant", content: "👋 Hello! I am your CS Tutor powered by Codevidhya. Ask me anything about coding, algorithms, programming languages, or computer science!" }
  ]);
        ...
        <div className="chat-window">
          <div className="chat-header">
            <div className="chat-header-info">
              ...
              <div><div className="chat-header-name">CS Tutor</div><div className="chat-header-sub">⚡ Powered by Codevidhya</div></div>
            </div>
...
export default function App() {
  ...
  const handleDownloadReport = () => {
    ...
    const errorsHtml = result.errors_found?.map((e, i) => `<li>${i+1}. ${e.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</li>`).join('') || '<li>No bugs found</li>';
    const fixesHtml  = result.fixes_applied?.map((f, i) => `<li>${i+1}. ${f.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</li>`).join('') || '<li>No fixes needed</li>';
    const outputSection = runOutput ? `<h2>▶ Run Output (${(runOutput.language||result.language).replace(/</g,'&lt;')})</h2><pre>${(runOutput.output||runOutput.error||'(no output)').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre>` : '';
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Debug Report — ${date}</title>
<style>body{font-family:'Segoe UI',Arial,sans-serif;max-width:900px;margin:0 auto;padding:40px;color:#1a1a2e}
h1{color:#2563eb;border-bottom:3px solid #2563eb;padding-bottom:10px}
h2{color:#1e40af;margin-top:28px;font-size:17px}
pre{background:#f1f5f9;padding:16px;border-radius:8px;font-size:12.5px;white-space:pre-wrap;word-break:break-all;border-left:4px solid #2563eb;font-family:monospace}
ul{padding-left:20px}li{margin:6px 0;line-height:1.6}
.meta{background:#eff6ff;padding:12px 16px;border-radius:8px;margin:16px 0;font-size:14px}
.footer{margin-top:40px;font-size:12px;color:#64748b;border-top:1px solid #e2e8f0;padding-top:12px;text-align:center}
@media print{body{padding:20px}}</style></head><body>
<h1>🔮 Code Debugging Report</h1>
<div class="meta"><strong>Date:</strong> ${date} &nbsp;&nbsp; <strong>Language:</strong> ${result.language} &nbsp;&nbsp; <strong>Bugs Found:</strong> ${result.errors_found?.length||0}</div>
<h2>📋 Original Code</h2>
<pre>${(result.original_code||'').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre>
<h2>🐛 Bugs Found</h2><ul>${errorsHtml}</ul>
<h2>🔧 Fixes Applied</h2><ul>${fixesHtml}</ul>
<h2>✅ Fixed Code</h2>
<pre>${(result.debugged_code||'').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre>
<h2>💡 Explanation</h2>
<p style="line-height:1.8;font-size:14px">${(result.explanation||'').replace(/\n/g,'<br>')}</p>
${outputSection}
<div class="footer">Generated by Coding Assistant · Powered by Codevidhya</div>
</body></html>`;
          ...
          <section className="panel">
            ...
            <div className="panel-footer">
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button type="button" style={{ display: "none" }}>{shareMsg}
                </button>
              </div>
              {runOutput && (
              ...
              {!result && !loading && (
                <div className="empty-state">
                  ...
                  <div className="empty-hints">
                    ...
                    <div className="hint-pill">⚡ Powered by Codevidhya</div>
                  </div>
                </div>
              )}
                ...
                <div className="result-body">
                  <div className="result-stats">
                    ...
                    <button onClick={handleDownloadReport}
                      style={{ padding: "4px 12px", background: "#0f766e", color: "white", border: "none",
                        borderRadius: 6, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
                      📄 Download Report
                    </button>
                  </div>
